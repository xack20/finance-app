...
| M4 | iOS DB encryption: SQLCipher **wired**, pending simulator-verify | P0b T3; M4 sweep | Now configured for real encryption: `encryptionConfig = Encryption(key)` (sqliter 1.3.3) + SQLDelight `linkSqlite=false` (framework defers sqlite) + `iosApp/Podfile` `pod 'SQLCipher' ~> 4.5` (static) provides `sqlite3_*` so `PRAGMA key` encrypts. Verified: framework re-links + `pod install` (SQLCipher 4.10.0) succeed. **NOT yet run on a simulator** — `xcodebuild -workspace` is blocked in this env because only the iOS 26.4 sim runtime is installed and Xcode 26.5's Pods build demands 26.5. To finish: `xcodebuild -downloadPlatform iOS` (install 26.5 runtime) → `cd iosApp && pod install` → `xcodebuild -workspace iosApp.xcworkspace -scheme iosApp -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17' build`, then confirm the DB file header is NOT plaintext `SQLite format 3` | P0d / M4-0 |
