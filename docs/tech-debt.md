...
| H6 | ~~16 KB page-size alignment — `libsodium.so`, `libsqlcipher.so`, … not aligned~~ **RESOLVED (2026-05-31, 16 KB dep bumps)** | P0b emulator test; M3-4; sim-verification 2026-05-31 | Only two libs were actually misaligned; fixed by bumping them: `net.zetetic:sqlcipher-android` **4.5.4 → 4.16.0** (16 KB-aligned since 4.6.1) and ionspin `multiplatform-crypto-libsodium-bindings` **0.9.3 → 0.9.5** (aligned since 0.9.4). The other three flagged libs — JNA `libjnidispatch` (5.18.1), `androidx.graphics.path` (1.0.1 via Compose), MediaPipe `tasks-genai` (0.10.24) — were **already** 16 KB-aligned (their dialog entries were "Unknown error", not "LOAD segment not aligned"). **Verified:** OS compat dialog no longer appears on fresh install + launch; all 5 `.so` LOAD segments now `0x4000` (16 KB) by ELF inspection; instrumented 38/0 + unit 323/0 + iOS compile green | done |
...
| S3 | Espresso 3.5.0 (transitive) `InputManager.getInstance` `NoSuchMethodException` broke all Compose instrumented tests on API 37 | Pinned `espresso-core 3.7.0` on `androidInstrumentedTest` | **Verified 2026-05-31:** full instrumented suite runs green on Pixel 10 Pro / API 37 — **38 tests, 0 failures** (the espresso fix also unmasked a latent `CaptureServiceBackfill` provider-hosting bug, fixed via in-process `MockContentResolver` — PR #2) |
