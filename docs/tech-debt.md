...
| H6 | 16 KB page-size alignment — `libsodium.so`, `libsqlcipher.so`, `libjnidispatch.so`, `libandroidx.graphics.path.so`, and MediaPipe's `libllm_inference_engine_jni.so` not aligned | P0b emulator test; M3-4; **re-confirmed M4 sim-verification 2026-05-31** | Runs in compatibility mode on Android 15+ devices; breaks under future Android requiring 16 KB. **Confirmed 2026-05-31:** launching the debug APK on the Pixel 10 Pro (API 37) emulator surfaced the OS *“This app isn't 16 KB compatible — ELF alignment check failed”* dialog naming exactly these 5 libs; the app still ran in page-size-compat mode and rendered the Welcome screen. Google Play requires 16 KB support for apps targeting Android 15+ — recompile native deps (NDK r28+ / updated lib versions) before that gate | P0d |
